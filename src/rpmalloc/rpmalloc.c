...
_memory_global_cache_insert(span_t* first_span, size_t list_size, size_t page_count)
{
	...
	while (1)
	{
		...
		if (global_span_ptr != SPAN_LIST_LOCK_TOKEN)
		{
			...
			size_t cache_limit = GLOBAL_SPAN_CACHE_MULTIPLIER * (_memory_max_allocation[span_class_idx] /
...
#ifdef GLOBAL_SPAN_CACHE_MULTIPLIER
			size_t cache_limit = GLOBAL_SPAN_CACHE_MULTIPLIER * (_memory_max_allocation_large[span_count - 1] /
																 MAX_SPAN_CACHE_DIVISOR);
